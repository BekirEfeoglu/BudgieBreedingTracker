...
- **Post edit — implement edildi (`feature/community-tab-faz1`, 2026-07-03; merge + prod migration apply bekliyor):** İçerik-yalnızca düzenleme 5 dk pencerede. `CommunityPostRepository.update({postId, content})` → `CommunityPostRemoteSource.updateContent` → `create-community-post` edge fn `mode:'update'` (moderation yeniden çalışır, fail-closed). Pencere edge fn'de (`EDIT_WINDOW_MS`) + `community_posts` authenticated UPDATE grant'i `(is_deleted, needs_review)` kolonlarına daraltılarak (migration `20260703120000`, prod'a 2026-07-03'te uygulandı) enforce edilir — content doğrudan client `.update()` ile değişemez. `edited_at` kolonu + UI'da `edited` rozeti; "Düzenle" yalnız kendi postunda + pencere içinde. (Yazarın kendi `needs_review`'ünü temizleyebilmesi bilinçli kapsam dışı — bkz. IMPROVEMENT_PLAN P3. Ayrıca `clearReviewFlag` var olmayan `reviewed_by` kolonuna yazıyor — pre-existing latent bug, IMPROVEMENT_PLAN §6.14.)

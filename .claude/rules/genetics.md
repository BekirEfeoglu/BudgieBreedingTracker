...
| v9 | 2026-07-25 | Multi-locus combiner (`_resolveEpistasisForCombined`) artık maskeleme SONRASI aynı compound adı paylaşan genotipik olarak FARKLI state'leri tek gruba çökertmiyor. Gruplama anahtarı v5'in double-factor set'inin yanına tam görsel/maskeli kimliği (`sorted(visual)/sorted(masked)`) da alır ve merge dalı `visualMutations`/`maskedMutations`'ı üzerine yazmak yerine **union**'lar. Önceki davranışta örn. görsel-ino cross'ta heterozigot dominant Blackface (masked `[blackface]`) ile düz wild-type (masked `[]`) ikisi de "Lutino" adına çözülüp aynı key'e merge oluyor, gizli-gen okuması son iterasyona göre yazılıyordu. **Olasılıklar ve fenotip adları zaten doğruydu**; değişen şey raporlanan görsel/maskeli mutasyon listeleri ve tek bir birleşik grubun ayrı gizli-gen state'lerine bölünebilmesi. Tek-lokus yolu etkilenmedi (ham fenotipe key'ler, listeleri zaten union'lıyordu). Test: `test/domain/services/genetics/multi_locus_masking_test.dart` |

Güncel değer: `GeneticsConstants.calculationVersion = 9`. `GeneticsHistory.isStale` bu sabite göre eski kayıtları işaretler; sabit değiştiğinde `genetics_constants_test.dart` literal assertion'ı ve `genetics_history_model_test.dart` isStale testleri güncellenmeli.

**2026-07-12 istisna — bump edilmedi:** Explicit linkage phase kontrolü (§ Sex-Linked Linkage) `calculationVersion`'ı DEĞİŞTİRMEDİ (o gün `8` kaldı). Bump kuralı algoritmanın *varsayılan* çıktısı değiştiğinde uygulanır; `LinkagePhase.auto` (varsayılan) byte-eşit kalır, yeni davranış yalnızca kullanıcının açıkça `coupling`/`repulsion` seçmesiyle tetiklenir — bu yeni bir GİRDİ, geriye dönük sessiz veri kayması değil. Persisted `GeneticsHistory.fatherPhaseOverrides` bu yüzden kendi `isStale` semantiğine ihtiyaç duymaz.
...
- Regression: `test/domain/services/genetics/` altında ~1.001 açık `test()` bildirimi (2026-07-25 envanteri, 52 dosya; parametrik çoğalmalar hariç) — güncel sayı için dosyaları say (`rg -c "^\s*test\(" test/domain/services/genetics/*.dart`), bu satırı sabit kalite hedefi olarak kullanma
- Linkage: 6 çiftin tamamı için coupling + repulsion `closeTo` assertion'ları (`genetics_linkage_test.dart`)
- **MUTAVI referans matrisi** (`mutavi_reference_regression_test.dart`): `docs/muhabbet-kusu-genetik-rehberi.md`'nin kanonik crosslarını (Blue×Blue/split, Cinnamon/Ino × normal dişi, Greywing×Clearwing/Dilute, Cinnamon+Ino→Lacewing) her fixture'ın `guideSection` + `sourceIds` (rehberin kaynak aralığı **K1–K15**; K15 Feather Duster/OMIA atfı) taşıdığı, rehber oranlarını `closeTo` ile doğrulayan tek matris. Motor rehberle çelişirse bu suite kırılır. Not: rehber genotip oranı (25/50/25) ile motorun **fenotip** çıktısı (25 görsel / 75 normal-taşıyıcı) split×split'te ayrışır — motor fenotip tahmin eder

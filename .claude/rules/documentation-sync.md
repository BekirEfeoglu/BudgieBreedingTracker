...
python3 scripts/check_rule_symbol_drift.py --strict  # every `xProvider`/`.dart` path named in .claude/rules/ must exist in code
```
...
**Aspirational-contract guard** (`check_rule_symbol_drift.py`, blocking in `code-quality`): catches the highest-value drift class — a rule naming a `fooProvider` or `.dart` path that no longer exists (the 2026-07-13 sweep found whole never-built rule sections, e.g. `conflictNotifierProvider`/`gamificationServiceProvider`). It ONLY checks Provider tokens and `.dart` paths (near-zero false-positive shapes); PascalCase classes, methods, l10n keys, and table/column names still need the manual semantic pass. A red means one of: (a) genuine drift — fix the rule to the real symbol; (b) a legitimately-removed symbol you're documenting in prose — add it to the `PROVIDER_ALLOWLIST`/`DART_PATH_ALLOWLIST` in the script with a one-line reason. Do NOT weaken the check to go green.

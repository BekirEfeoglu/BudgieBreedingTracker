        ...
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              {birdInfo.ageInfo && (() => {
                // bird objesinin herhangi bir alanında 'birds.ageCategory' içeren bir string var mı kontrol et
                const rawAgeCategory = Object.values(bird).find(
                  v => typeof v === 'string' && v.includes('birds.ageCategory')
                ) as string | undefined;
                const extracted = extractAgeCategory(rawAgeCategory || '');
                const normalizedAgeCategory = extracted ? normalizeAgeCategory(extracted) : getAgeCategoryLabel(bird.birthDate!, t);
                return (
                  <span 
                    className="text-lg flex-shrink-0" 
                    aria-label={`${resolveAgeCategoryKey(normalizedAgeCategory, t)} yaş kategorisi`}
                    title={resolveAgeCategoryKey(normalizedAgeCategory, t)}
                  >

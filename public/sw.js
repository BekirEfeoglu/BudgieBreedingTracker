// Service Worker for Budgie Breeding Tracker PWA
const CACHE_NAME = 'budgie-tracker-v2';
const STATIC_CACHE = 'budgie-static-v2';
const DYNAMIC_CACHE = 'budgie-dynamic-v2';
const IMAGE_CACHE = 'budgie-images-v2';
const API_CACHE = 'budgie-api-v2';
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification('Budgie Tracker', options)
  );
});
...
async function cleanupCache() {
  try {
    ...
    for (const cacheName of cacheNames) {
      if (cacheName.startsWith('budgie-')) {

...
Bird _bird(String id, String name, BirdGender gender) => Bird(
  id: id, userId: 'test-user', name: name, gender: gender,
...
void main() {
  ...
  setUp(() {
    router = GoRouter(
      ...
      routes: [
        GoRoute(path: '/breeding', builder: (_, __) => const Text('B'), routes: [
          GoRoute(
            path: 'form',
  ...
  group('Empty state handling', () {
    testWidgets('no available birds shows empty state', (t) async {
      await t.pumpWidget(_subject(birds: Stream.value([])));
    ...
    testWidgets('form renders with only male birds (no females)', (t) async {
      ...
      await t.pumpWidget(_subject(birds: Stream.value(males), males: males));
      await t.pumpAndSettle();
    ...
    testWidgets('form renders with only female birds (no males)', (t) async {
      ...
      await t.pumpWidget(
        _subject(birds: Stream.value(females), females: females),
      );
      await t.pumpAndSettle();
      expect(find.byType(Form), findsOneWidget);
      expect(find.textContaining('(0)'), findsAtLeastNWidgets(1));
    ...
    testWidgets('submit without selecting birds shows validation error',
      ...
      await t.tap(btn);
      await t.pumpAndSettle();
      expect(find.textContaining('validation.field_required'),

...
void main() {
  ...
  group('profileCompletionProvider', () {
    test('computes completion percentage from profile and stats', () async {
      const profile = Profile(
        id: userId,
        email: 'owner@example.com',
        fullName: 'Owner',
      ...
      final container = ProviderContainer(
        overrides: [
          userProfileProvider.overrideWith((_) => Stream.value(profile)),
          profileStatsProvider(userId).overrideWithValue(
            const AsyncData(
              ProfileStats(
                totalBirds: 3,
                totalPairs: 1,
      ...
      final completion = container.read(profileCompletionProvider(userId));

      expect(completion.percentage, closeTo(1.0, 0.0001));
      expect(completion.items.every((item) => item.isCompleted), isTrue);
    });

    test(
      'leaves incomplete items false when profile and stats are empty',
      () async {
        const profile = Profile(id: userId, email: 'owner@example.com');
        ...
        final container = ProviderContainer(
          overrides: [
            ...
            profileStatsProvider(
              userId,
            ).overrideWithValue(const AsyncData(ProfileStats())),
          ],
        );
        addTearDown(container.dispose);
        ...
        expect(completion.percentage, closeTo(0.0, 0.0001));
        expect(completion.items.where((item) => item.isCompleted), isEmpty);
      },
    );
  });
}

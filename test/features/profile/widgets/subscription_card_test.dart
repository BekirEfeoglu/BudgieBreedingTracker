...
void main() {
  group('SubscriptionCard', () {
    testWidgets('renders without crashing for null profile', (tester) async {
      await tester.pumpWidget(_wrap(const SubscriptionCard(profile: null)));
      await tester.pump();
    ...
    testWidgets('shows upsell card when profile has no premium', (
      ...
      await tester.pumpWidget(_wrap(const SubscriptionCard(profile: profile)));
      await tester.pump();
    ...
    testWidgets('shows premium status card for premium user', (tester) async {
      ...
      await tester.pumpWidget(_wrap(SubscriptionCard(profile: profile)));
      await tester.pump();

      // Consume possible Row overflow from member tenure line (clock icon + text)
    ...
    testWidgets('shows manage button for premium user', (tester) async {
      ...
      // Consume possible Row overflow from member tenure line
      final ex = tester.takeException();
      if (ex != null && !ex.toString().contains('overflowed')) {
        throw ex as Object;
      }

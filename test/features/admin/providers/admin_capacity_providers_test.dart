...
class _FakeCapacityClient extends Fake implements SupabaseClient {
  _FakeCapacityClient({required this.adminQueryBuilder, this.rpcResult});
  ...
  PostgrestFilterBuilder<T> rpc<T>(
    ...
    return _FakeRpcBuilder<T>(result: rpcResult as T?);
  }
}
...
void main() {
  ...
  group('ServerCapacity critical threshold detection', () {
    ...
    test('above 90% DB threshold is critical', () {
      ...
      expect(
        dbRatio,
        greaterThanOrEqualTo(AdminConstants.capacityWarningPercent),
      );
    });
  ...
  group('AdminConstants.dbSizeLimitForPlan', () {
    test('returns 500 MB for free plan', () {
      expect(AdminConstants.dbSizeLimitForPlan('free'), 500 * 1024 * 1024);
    });

    test('returns 8 GB for pro plan', () {
      expect(AdminConstants.dbSizeLimitForPlan('pro'), 8 * 1024 * 1024 * 1024);
    });

    test('returns 8 GB for team plan', () {
      expect(AdminConstants.dbSizeLimitForPlan('team'), 8 * 1024 * 1024 * 1024);
    });

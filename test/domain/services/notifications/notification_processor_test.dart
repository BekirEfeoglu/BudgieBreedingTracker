...
void main() {
  setUpAll(() {
    registerFallbackValue(NotificationSchedule(
      id: '',
      userId: '',
  ...
  group('NotificationProcessor.processEventReminders', () {
    ...
    test('schedules future reminder via service', () async {
      ...
      when(() => remindersDao.countUnsent('user-1'))
          .thenAnswer((_) async => 1);
      when(() => remindersDao.getUnsent('user-1'))
          .thenAnswer((_) async => [reminder]);
      when(() => eventsDao.getById('evt-1')).thenAnswer((_) async => event);
      when(() => service.scheduleNotification(
            id: any(named: 'id'),
      ...
      verify(() => service.scheduleNotification(
            id: any(named: 'id'),
            title: 'Vet Visit',
            body: any(named: 'body'),
            scheduledDate: any(named: 'scheduledDate'),
    ...
    test('shows immediate notification for recent past reminder', () async {
      ...
      when(() => scheduler.showImmediateNotification(
            id: any(named: 'id'),
            title: any(named: 'title'),
            body: any(named: 'body'),
            type: any(named: 'type'),
      ...
      verify(() => scheduler.showImmediateNotification(
            id: any(named: 'id'),
            title: 'Feeding',
            body: any(named: 'body'),
            type: 'event_reminder',
    ...
    test('skips reminder when event not found', () async {
      ...
      when(() => remindersDao.getUnsent('user-1'))
          .thenAnswer((_) async => [reminder]);
      when(() => eventsDao.getById('evt-missing'))
    ...
    test('continues processing when individual reminder fails', () async {
      ...
      when(() => eventsDao.getById('evt-fail'))
          .thenThrow(Exception('DB error'));
      when(() => eventsDao.getById('evt-ok'))
  ...
  group('NotificationProcessor.processNotificationSchedules', () {
    ...
    test('schedules future notification via service', () async {
      ...
      when(() => schedulesDao.countPending('user-1'))
          .thenAnswer((_) async => 1);
      when(() => schedulesDao.getPending('user-1'))
      ...
      when(() => schedulesDao.markProcessed('sched-1'))
          .thenAnswer((_) async {});
      ...
      verify(() => schedulesDao.markProcessed('sched-1')).called(1);
    });
    ...
    test('shows immediate for recent past schedule', () async {
      ...
      when(() => schedulesDao.markProcessed('sched-2'))
          .thenAnswer((_) async {});
    ...
    test('marks old schedule as processed without showing', () async {
      ...
      when(() => schedulesDao.getPending('user-1'))
          .thenAnswer((_) async => [schedule]);
      when(() => schedulesDao.markProcessed('sched-3'))
          .thenAnswer((_) async {});
    ...
    test('creates next occurrence for recurring schedule', () async {
      ...
      when(() => schedulesDao.markProcessed('sched-recurring'))
          .thenAnswer((_) async {});
      ...
      final captured =
          verify(() => schedulesDao.insertItem(captureAny())).captured;
    ...
    test('does not create next occurrence when interval would be in past',
        () async {
      ...
      when(() => schedulesDao.getPending('user-1'))
          .thenAnswer((_) async => [schedule]);
      when(() => schedulesDao.markProcessed('sched-old-recurring'))
          .thenAnswer((_) async {});
    ...
    test('continues processing when individual schedule fails', () async {
      ...
      when(() => schedulesDao.markProcessed('sched-ok'))
          .thenAnswer((_) async {});
      ...
      verify(() => schedulesDao.markProcessed('sched-ok')).called(1);
    });
  });
  ...
  group('NotificationProcessor.processAll', () {
    test('runs reminders, schedules and cleanup in parallel', () async {
      ...
      when(() => remindersDao.countUnsent(any()))
          .thenAnswer((_) async => 0);
      when(() => schedulesDao.countPending(any()))
          .thenAnswer((_) async => 0);
      when(() => settingsDao.getByUser(any()))
          .thenAnswer((_) async => null);
      when(() => notificationsDao.deleteOldRead(any(), daysOld: any(named: 'daysOld')))
      ...
      expect(
        verify(() => remindersDao.countUnsent(any())).callCount,
        1,
      );
      expect(
        verify(() => schedulesDao.countPending(any())).callCount,
        1,
      );
      verify(
    ...
    test('cleanup uses custom cleanupDaysOld from settings', () async {
      ...
      verify(
        () => notificationsDao.deleteOldRead(any(), daysOld: 7),
      ).called(1);
    });
    ...
    test('cleanup does not break processAll when it fails', () async {
      ...
      when(() => settingsDao.getByUser(any()))
          .thenAnswer((_) async => null);
      when(() => notificationsDao.deleteOldRead(any(), daysOld: any(named: 'daysOld')))
          .thenAnswer((_) async { throw Exception('DB locked'); });

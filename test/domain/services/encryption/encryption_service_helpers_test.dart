...
void main() {
  ...
  group('Key management helpers', () {
    group('_getOrCreateKeyBytes', () {
      ...
      test(
        'regenerates key when stored key is corrupted (too short)',
        () async {
          ...
          when(
            () => mockStorage.read(key: _keyVersionName),
          ).thenAnswer((_) async => null);
    ...
    group('_zeroCachedKey (via dispose/deleteKey)', () {
      test(
        'dispose clears cache and forces re-read on next operation',
        () async {
          when(
            () => mockStorage.read(key: _keyName),
          ).thenAnswer((_) async => _validBase64Key);
    ...
    group('_loadPreviousKeys (via decrypt fallback)', () {
      test(
        'falls back to previous key version when active key fails',
        () async {
          // Key used for original encryption
          final originalKeyBytes = Uint8List.fromList(
          ...
          when(
            () => mockStorage.read(key: '${_previousKeyPrefix}0'),
      ...
      test('throws when no previous key can decrypt the data', () async {
        ...
        await expectLater(service.decrypt(badCipher), throwsA(anything));
      });

      test('skips corrupted archived keys during fallback', () async {
        final newKeyBytes = Uint8List.fromList(
          List<int>.generate(32, (i) => 200 + (i % 56)),
        );
        final newBase64 = base64Encode(newKeyBytes);
        ...
        when(
          () => mockStorage.read(key: '${_previousKeyPrefix}1'),
    ...
    group('reEncrypt', () {
      test(
        're-encrypts successfully and produces different ciphertext',
        () async {
          when(
            () => mockStorage.read(key: _keyName),
          ).thenAnswer((_) async => _validBase64Key);

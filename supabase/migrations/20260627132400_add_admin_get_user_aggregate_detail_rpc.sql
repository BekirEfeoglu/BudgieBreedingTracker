-- Create an RPC to fetch all user detail counts and data in a single network roundtrip.
CREATE OR REPLACE FUNCTION admin_get_user_aggregate_detail(p_user_id uuid)
RETURNS json
LANGUAGE plpgsql
...
DECLARE
  ...
  v_result json;
BEGIN
  -- Verify admin access (assuming is_admin() function exists and checks role)
  -- The is_admin() function is already defined in earlier migrations.

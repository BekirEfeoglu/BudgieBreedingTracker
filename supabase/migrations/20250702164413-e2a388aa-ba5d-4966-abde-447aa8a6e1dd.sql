-- Performance optimizations: Add compound indexes for better query performance
...
-- 7. Partial indexes for active/recent data (better performance for common queries)
CREATE INDEX IF NOT EXISTS idx_birds_recent_active ON public.birds(user_id, created_at) 
WHERE created_at > (NOW() - INTERVAL '1 year');

CREATE INDEX IF NOT EXISTS idx_incubations_active ON public.incubations(user_id, start_date) 
WHERE start_date > (NOW() - INTERVAL '6 months');

CREATE INDEX IF NOT EXISTS idx_eggs_active_laid ON public.eggs(user_id, lay_date) 
WHERE status IN ('laid', 'fertile') AND lay_date > (NOW() - INTERVAL '3 months');

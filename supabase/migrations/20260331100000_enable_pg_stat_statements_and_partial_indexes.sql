...
-- block, where CREATE INDEX CONCURRENTLY is not allowed.
CREATE INDEX IF NOT EXISTS idx_birds_active
  ON birds (user_id) WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_eggs_active
  ON eggs (user_id) WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_chicks_active
...
CREATE INDEX IF NOT EXISTS idx_clutches_active
  ON clutches (user_id) WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_nests_active
...
CREATE INDEX IF NOT EXISTS idx_genetics_history_father_mutations_gin
  ON genetics_history USING gin (father_mutations jsonb_path_ops);

CREATE INDEX IF NOT EXISTS idx_genetics_history_mother_mutations_gin

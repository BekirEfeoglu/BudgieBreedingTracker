-- Premium Abonelik Sistemi Migration
-- Bu migration premium abonelik sistemi için gerekli tabloları ve fonksiyonları oluşturur
...
-- 3. SUBSCRIPTION_USAGE TABLOSU (Kullanım istatistikleri)
...
-- 4. SUBSCRIPTION_EVENTS TABLOSU (Abonelik olayları)
...
-- 5. PROFILES TABLOSUNA PREMIUM ALANLARI EKLEME
...
-- TRIGGER'LAR
-- Önce mevcut trigger'ları sil (eğer varsa)
...
-- VARSayılan PLANLAR
INSERT INTO public.subscription_plans (name, display_name, description, price_monthly, price_yearly, currency, features, limits) 
SELECT 
  'free',
  'Ücretsiz',
...
-- Profiles tablosu için premium alanları güncelleme politikası
...
-- Varsayılan kullanıcıları ücretsiz plana ata
UPDATE public.profiles 
SET subscription_status = 'free'
WHERE subscription_status IS NULL;

-- REALTIME PUBLICATION AYARLARI
-- Tabloları güvenli şekilde publication'a ekle
DO $$
BEGIN
    -- subscription_plans
    IF NOT EXISTS (
...
SELECT 'Premium subscription system migration completed successfully' as message; 
-- Fix schema inconsistencies in eggs and chicks tables (v4)
...
-- If there are still chicks without incubation_id, create a default incubation for each user
...
SELECT DISTINCT 
  ...
  CASE 
    WHEN c.father_id IS NOT NULL AND c.mother_id IS NOT NULL 
    THEN c.father_id::text || '_' || c.mother_id::text
    WHEN c.father_id IS NOT NULL 
    THEN c.father_id::text || '_unknown'
    WHEN c.mother_id IS NOT NULL 
    THEN 'unknown_' || c.mother_id::text
    ELSE 'unknown_pair'
  END,
...
-- Now make incubation_id NOT NULL for chicks
ALTER TABLE public.chicks 
ALTER COLUMN incubation_id SET NOT NULL;

-- 3. Add check constraint to ensure data consistency for eggs
DO $$ 
BEGIN
  -- Check if constraint already exists before adding
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'eggs_must_have_clutch_or_incubation'
  ) THEN
    ALTER TABLE public.eggs 
...
-- Apply the egg validation trigger
...
-- Apply the chick validation trigger  

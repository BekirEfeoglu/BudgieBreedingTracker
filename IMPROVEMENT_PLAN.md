...
| 6.14 | **`clearReviewFlag` var olmayan `reviewed_by` kolonuna yazıyor (admin moderation)** | 2026-07-03 migration-apply anında keşfedildi: `community_post_remote_source.dart:264` `clearReviewFlag`, `community_posts`'ta HİÇ var olmayan `reviewed_by` kolonuna yazıyor (hiçbir migration eklememiş) → admin "incelemeyi temizle" aksiyonu prod'da PGRST hatasıyla kırık (pre-existing, community-tab branch'inden bağımsız). Karar gerekli: (a) client'tan `reviewed_by` yazımını kaldır (audit izi yok, minimal), VEYA (b) `reviewed_by uuid` kolonu + `reviewed_at` ekle (audit izi korunur). Edit-hardening grant'i bu kolonu bilinçli granta ETMEDİ. |

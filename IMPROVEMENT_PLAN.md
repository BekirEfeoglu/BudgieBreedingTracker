...
| 6.13 | ✅ **ÇÖZÜLDÜ (2026-07-03, `fix/community-followups`)** | Edit sheet artık `showAppBottomSheet` kullanıyor (SafeArea alt-inset); yorum empty-state `visibleComments.isEmpty` ile tetikleniyor (tam-mute thread boş görünmüyor). Ek: `edit_window_expired` mesajı yüzeye çıkarıldı, redundant cache invalidate kaldırıldı, edit-menü tap-through testi eklendi. |
| 6.14 | ✅ **ÇÖZÜLDÜ (2026-07-03, `fix/community-followups`)** | `clearReviewFlag`'in var olmayan `reviewed_by` kolonuna yazımı kaldırıldı (seçenek (a) — audit izi zaten `audit_logs` tablosunda). Admin "incelemeyi temizle" artık yalnız `needs_review=false` yazıyor, prod'da çalışıyor. `community_posts`'a `reviewed_by` kolonu eklenmedi (edit-hardening grant'i de zaten bilinçli granta etmemişti). |

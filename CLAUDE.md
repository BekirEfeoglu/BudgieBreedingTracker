...
| Test files (test/) | 861 test files, 10,651+ individual tests |
| Feature modules | 24 |
...
| L10n keys | ~2,810 per language, 41 categories |
...
### Migrations
149 SQL migration files in `supabase/migrations/`. Schema managed server-side; never modify RLS policies from client code.
...
Migrations:    supabase/migrations/ (149 files)
Scripts:       scripts/

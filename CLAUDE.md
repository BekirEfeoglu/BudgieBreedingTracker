...
- **Localization**: `'feature.key'.tr()` — master file is `tr.json` (~1,732 keys, 34 categories)
- **Database**: Schema version 12, migration via for-loop + switch pattern
...
- **Supabase**: Table/column names via `SupabaseConstants` (59 constants), never hardcoded
...
- `navigation.md` — GoRouter routes (52), guards, edit mode, route ordering

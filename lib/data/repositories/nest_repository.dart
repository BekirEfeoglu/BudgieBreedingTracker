...
class NestRepository extends BaseRepository<Nest>
    with SyncableRepository<Nest> {
  ...
  @override
  Stream<List<Nest>> watchAll(String userId) => _localDao.watchAll(userId);

  @override
  Stream<Nest?> watchById(String id) => _localDao.watchById(id);

  @override
  Future<List<Nest>> getAll(String userId) => _localDao.getAll(userId);

  @override
  Future<Nest?> getById(String id) => _localDao.getById(id);

  @override
  Future<void> save(Nest item) async {
  ...
  @override
  Future<void> saveAll(List<Nest> items) async {
  ...
  @override
  Future<void> push(Nest item) async {
    try {
  ...
  Future<PushStats> pushAll(String userId) async {
    ...
    for (final meta in tablePending) {
      ...
      if (item == null) {
        AppLogger.warning('[NestRepo] Orphan sync_metadata cleaned: ${meta.recordId}');
  ...
  Future<List<Nest>> getAvailable(String userId) =>
      _localDao.getAvailable(userId);

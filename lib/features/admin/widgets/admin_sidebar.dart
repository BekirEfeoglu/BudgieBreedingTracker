...
final _securityBadgeCountProvider = FutureProvider.autoDispose<int>((
  ref,
) async {
  ...
  try {
    ...
    return events.where((e) => e.severity == SecuritySeverityLevel.high).length;
  } catch (e) {
...
final _feedbackBadgeCountProvider = FutureProvider.autoDispose<int>((
  ref,
) async {
  await requireAdmin(ref);
  final client = ref.watch(supabaseClientProvider);
  ...
  try {
    final count = await client
        ...
        .count()
        .or(
          'status.eq.${FeedbackStatus.open.toJson()},status.eq.${FeedbackStatus.pending.toJson()}',
        );
...
class AdminSidebar extends ConsumerWidget {
  ...
  int _getBadgeCount(WidgetRef ref, String key) {
    switch (key) {
      case 'security':
        return ref
            .watch(_securityBadgeCountProvider)
            .maybeWhen(data: (count) => count, orElse: () => 0);
      case 'feedback':
        return ref
            .watch(_feedbackBadgeCountProvider)
            .maybeWhen(data: (count) => count, orElse: () => 0);
      default:
        return 0;
    }

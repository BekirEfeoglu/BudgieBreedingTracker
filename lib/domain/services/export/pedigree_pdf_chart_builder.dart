...
class PedigreePdfChartBuilder {
  ...
  pw.Widget buildChart(Bird root, Bird? father, Bird? mother, Bird? pgf,
      Bird? pgm, Bird? mgf, Bird? mgm) {
    return pw.Container(
      height: 260,
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.Expanded(
                ...
                _gpCell(pgm, bottomBorder: PdfColors.grey600, thick: true),
  ...
  pw.Widget buildStats(
      int found, int possible, int deepestGen, double completeness) {
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          _statDivider(),
          _statItem('genealogy.completeness'.tr(),
              '%${completeness.toStringAsFixed(1)}'),
          _statDivider(),
          _statItem('genealogy.deepest_generation'.tr(),
  ...
  pw.Widget _gpCell(Bird? bird,
      {PdfColor? bottomBorder, bool thick = false}) {
    return pw.Expanded(
        child: pw.Container(
          ...
          ? pw.BoxDecoration(
              border: pw.Border(
                  bottom: pw.BorderSide(
                      color: bottomBorder, width: thick ? 1 : 0.5)))
          : null,
      child: pw.Center(child: _chartNode(bird, small: true)),
    ));
  }

  pw.Widget _chartNode(Bird? bird,
      {bool isRoot = false, bool small = false}) {
    if (bird == null) {
      return pw.Container(
          ...
          children: [
            pw.Text('?',
                style: pw.TextStyle(
                    ...
                    fontWeight: pw.FontWeight.bold)),
            pw.Text('genealogy.unknown_parent'.tr(),
                style: pw.TextStyle(
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          pw.Text(bird.name,
              style: pw.TextStyle(
                  fontSize: nameFontSize, fontWeight: pw.FontWeight.bold),
              maxLines: 1),
          if (bird.ringNumber != null)
            pw.Text(bird.ringNumber!,
                style: pw.TextStyle(
                    fontSize: subFontSize, color: PdfColors.grey700)),
          pw.Text(
            '${PedigreePdfHelpers.genderLabel(bird.gender.name)} · ${PedigreePdfHelpers.statusLabel(bird.status.name)}',
            style:
                pw.TextStyle(fontSize: subFontSize, color: PdfColors.grey600),
          ),
          if (isRoot && bird.birthDate != null)
            pw.Text(PedigreePdfHelpers.dateFormat.format(bird.birthDate!),
                style: pw.TextStyle(
                    fontSize: subFontSize, color: PdfColors.grey600)),
        ],
      ),
    );
  }

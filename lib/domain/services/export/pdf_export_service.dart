...
class PdfExportService {
  ...
  pw.Page _buildBirdsPage(List<Bird> birds) {
    return pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      header: (context) => _header('export.section_birds'.tr(args: ['${birds.length}'])),
      build: (context) => [
        pw.TableHelper.fromTextArray(
          ...
          headers: ['export.header_name'.tr(), 'export.header_ring_number'.tr(), 'export.header_gender'.tr(), 'export.header_status'.tr(), 'export.header_birth_date'.tr()],
          data: birds.map((b) {
            return [
              b.name,
              b.ringNumber ?? '-',
  ...
  pw.Page _buildBreedingPage(List<BreedingPair> pairs) {
    return pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      header: (context) => _header('export.section_breeding'.tr(args: ['${pairs.length}'])),
      build: (context) => [
        pw.TableHelper.fromTextArray(
          headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
          headerDecoration:
              const pw.BoxDecoration(color: PdfColors.grey200),
          cellPadding: const pw.EdgeInsets.all(4),
          headers: ['export.header_id'.tr(), 'export.header_cage'.tr(), 'export.header_status'.tr(), 'export.header_pairing_date'.tr(), 'export.header_separation_date'.tr()],
          data: pairs.map((p) {
            return [
              p.id.substring(0, 8),
              p.cageNumber ?? '-',
              p.status.name,
              p.pairingDate != null
                  ? _dateFormat.format(p.pairingDate!)
                  : '-',
  ...
  pw.Page _buildEggsPage(List<Egg> eggs) {
    return pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      header: (context) => _header('export.section_eggs'.tr(args: ['${eggs.length}'])),
      build: (context) => [
        pw.TableHelper.fromTextArray(
          headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
          headerDecoration:
              const pw.BoxDecoration(color: PdfColors.grey200),
          cellPadding: const pw.EdgeInsets.all(4),
          headers: ['export.header_no'.tr(), 'export.header_lay_date'.tr(), 'export.header_status'.tr(), 'export.header_hatch_date'.tr()],
          data: eggs.map((e) {
            return [
              '${e.eggNumber ?? "-"}',
  ...
  pw.Page _buildChicksPage(List<Chick> chicks) {
    return pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      header: (context) => _header('export.section_chicks'.tr(args: ['${chicks.length}'])),
      build: (context) => [
        pw.TableHelper.fromTextArray(
          headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
          headerDecoration:
              const pw.BoxDecoration(color: PdfColors.grey200),
          cellPadding: const pw.EdgeInsets.all(4),
          headers: ['export.header_name'.tr(), 'export.header_ring'.tr(), 'export.header_gender'.tr(), 'export.header_health'.tr(), 'export.header_hatch_date'.tr()],
          data: chicks.map((c) {
            return [
              c.name ?? '-',
              c.ringNumber ?? '-',
              ...
              c.healthStatus.name,
              c.hatchDate != null
                  ? _dateFormat.format(c.hatchDate!)
                  : '-',
            ];
          }).toList(),
        ),

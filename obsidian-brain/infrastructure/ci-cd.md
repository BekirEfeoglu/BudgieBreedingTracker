...
| `code-quality` | Anti-pattern scan + platform target policy + release-notes site freshness (`generate_release_notes_site.py --check`) + wiki lint + migration structure drift (`verify_migration_drift.py`) + rule symbol drift (`check_rule_symbol_drift.py --target all --classes --strict`, rules + wiki) | PR merge |
